...
from gloryxr.utils import (load_models, load_reaction_subsets, load_sdf_data,
                           print_summary, save_failed_molecules,
                           save_predictions, standardize_molecules)


def load_prediction_components() -> (
    tuple[dict[str, RandomForestClassifier], dict[str, str]]
):
    """
...
def run_prediction_pipeline(
    input_sdf: str, output_folder: str, strict_soms: bool
) -> int:
    """
    ...
    try:
        ...
        failed_molecules = (
            pd.concat([standardization_failed, prediction_failed], ignore_index=True)
            if not standardization_failed.empty or not prediction_failed.empty
            else pd.DataFrame()
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "-i", "--input_sdf", help="Path to input SDF file", type=str, required=True
    )
    parser.add_argument(
        "-o", "--output_folder", help="Path to output folder", type=str, required=True
    )
    parser.add_argument(
        "--strict_soms",
        help="Whether to use strict SOMs",
        action="store_true",
        default=False,
    )
