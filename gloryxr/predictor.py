...
class MetabolitePredictor:
    ...
    def __init__(
        ...
        print(f"Initialized reactor with {len(self.reactor.abstract_reactions)} reaction rules")
    ...
    def _generate_predictions(self, reactions_df: pd.DataFrame, parent_name: str) -> pd.DataFrame:
        """
        ...
        exploded_df["Descriptors"] = exploded_df["SOM"].map(self.vectorizer.transform_one)
        ...
        columns_to_keep = ["parent_name", "parent_smiles", "metabolite_smiles", 
                          "Reaction", "Subset", "SOM", "Score"]
